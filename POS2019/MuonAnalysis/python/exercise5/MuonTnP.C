...
void MuonTnP::Loop(float tagPt, float probePt, float mllDiff, float tagIso, float probeIso, int maxEvts)
{
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  // histograms are commended out - uncommend as the excercise continues
   ...
   for (Long64_t jentry=0; jentry<maxEntries;jentry++) {
      ...
      if(jentry % 1000 == 0)  std::cout << "Processed entries:" << jentry << std::endl; 
      ...
      //YOYR JOB loop in muons and find possible probes
